...
- 📱 **多屏桌面轮播与跟手滑动手势 (Multi-Desktop Carousel, v1.3.0)**：创新分屏桌面架构，集成 Embla 物理动能引擎，支持鼠标拖拽跟手横滑、键盘左右键快速切屏与底部毛玻璃指示点快捷跳转；支持分屏重命名、删除迁移，并具备分屏独立分类记忆与跨屏快速归类能力。
...
- 🎨 **纯粹图标模式与极简高质感设计 (v1.3.0)**：网格模式默认采用类似移动桌面的纯粹图标视图（隐藏卡片外框，55% 黄金图标占比），亦支持毛玻璃卡片背景、卡片透明度与大小自由微调；支持**时钟、日期、问候语、搜索栏、分类标签、卡片 6 大主页元素独立色彩定制**，搭配精选调色盘与 Hex 自定义色盘；内置 **Canvas 壁纸亮度分区自适应引擎**，智能探测明暗对比并自动施加动态微投影，深浅配色方案独立保存互不干扰。
