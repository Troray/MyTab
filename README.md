...
- 📱 **多屏桌面轮播与跟手滑动手势 (Multi-Desktop Carousel, v1.3.0)**：新增分屏桌面架构，集成 Embla 物理动能引擎，支持鼠标拖拽跟手横滑、键盘左右键快速切屏与底部毛玻璃指示点快捷跳转；支持分屏重命名、删除迁移，并具备分屏独立分类记忆与跨屏快速归类能力。
- 🔖 **浏览器书签一键原生导入与智能去重 (Bookmarks Importer, v1.3.0)**：全面支持毫秒级直读 Chrome、Edge、Firefox 等浏览器原生书签树（书签栏、其他书签等），亦兼容 Safari 及各大浏览器的 HTML 书签导出文件。支持可视化树形多选、智能展平映射、已存网址去重比对与导入前增量统计。
...
- 🎨 **纯粹图标模式与极简高质感设计 (v1.3.0)**：网格模式默认采用类似移动桌面的纯粹图标视图，亦支持毛玻璃卡片背景、卡片透明度与大小自由微调；支持**时钟、日期、问候语、搜索栏、分类标签、卡片 6 大主页元素独立色彩定制**，搭配精选调色盘与 Hex 自定义色盘；内置 **Canvas 壁纸亮度分区自适应引擎**，智能探测明暗对比并自动施加动态微投影，深浅配色方案独立保存互不干扰。
...
- 🗂️ **分类与拖拽重排**：支持多分组管理、平滑自然的卡片拖拽重排 (Drag & Drop) 与快捷增删改查。
...
- **`bookmarks`**：用于按需一键读取浏览器原生书签树（书签栏、其他书签），实现快捷可视化导入并映射至 MyTab，数据严格仅在本地离线解析，绝不上传。
...
- 🎠 **[Embla Carousel](https://www.embla-carousel.com/)**：轻量级、高扩展、具备物理惯性与跟手触摸特性的现代化无依赖轮播引擎。
